...
namespace MasonVeteransMemorial
{
    class MatrixDisplay
    {
        ...
        public void Reset(SKCanvas canvas, SKMatrix matrix, SKPoint location)
        {
            MeasureAndPaint(canvas, matrix, location, true, true);
        }
        ...
        SKSize MeasureAndPaint(SKCanvas canvas, SKMatrix matrix, SKPoint location, bool doPaint, bool reset = false)
        {
            SKMatrix resetMatrix = new SKMatrix();


            float[] values = matrix.Values;

            if (reset)
                values = resetMatrix.Values;
